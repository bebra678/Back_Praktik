...
<div class="back">
    ...
    <?php
    echo '<h1>' . $name . '</h1>';
    echo '<h2>Дисциплины:</h2>';
    $pod[0] = 'Пусто';
    foreach ($pod_dis as $el)
    {
        $pod[] = $el->id_dis;
    }
    $pod_unique = array_unique($pod);
    foreach ($disciplines as $el)
    {
        for($i = 1; $i <= count($pod_unique); $i++)
        {
            if($el->id == $pod_unique[$i])
            { ?>
                <form action="" method="post" class="login-form" enctype="multipart/form-data">
                    ...
                    <br><button class="lock" value="dis"><?= $el->name ?></button>
                </form>
    <?php }
        }
    }
    ...
    <div class="nazad"><a href="<?= app()->route->getUrl('/pod') ?>">Назад</a></div>
</div>
</body>
</html>